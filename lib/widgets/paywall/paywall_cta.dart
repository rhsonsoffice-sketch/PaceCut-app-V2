...
class PaywallCta extends StatefulWidget {
  /// Currently selected plan: 0 = Weekly (£2.99), 1 = Monthly (£5.99).
...
class _PaywallCtaState extends State<PaywallCta>
  ...
  Future<void> _handlePurchase() async {
    ...
    try {
      ...
      // On iOS/Android: result arrives asynchronously via onPurchaseSuccess /
      // onPurchaseError callbacks — _processing is cleared there.
      // On web: buyProduct calls onPurchaseError synchronously, which already
      // resets _processing. The finally only fires after both paths complete,
      // so we must NOT reset _processing here unconditionally — only reset it
      // if we're still processing (i.e. the async callback hasn't fired yet).
    } catch (_) {
      // Synchronous throw (e.g. platform channel not available) — reset spinner.
...
class _LegalLinkState extends State<_LegalLink> {
  ...
  String get _url => widget.label == 'Privacy Policy'
      ? 'https://forms.gle/smwNZbfTHmYRX9rE8'
