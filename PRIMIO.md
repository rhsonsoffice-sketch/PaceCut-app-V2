...
## Billing & PRO State
- Paywall pricing is GBP: Weekly £2.99/week, Monthly £5.99/month ("BEST VALUE" badge)
...
- `IapService` (singleton in `lib/services/iap_service.dart`) wraps `in_app_purchase` package — initialised in `main()` via `IapService.instance.init()`; callbacks `onPurchaseSuccess` / `onPurchaseError` wired in `_PaywallCtaState.initState()` and cleared in `dispose()` to prevent post-unmount calls; product IDs: `com.primio.pacecutai.weekly` and `com.primio.pacecutai.monthly` — must be registered in App Store Connect before going live
- Real StoreKit sheet is presented natively by iOS when `IapService.instance.buyProduct()` is called; "Restore Purchases" button calls `IapService.instance.restorePurchases()` (required by Apple Guideline 3.1.1)
...
- `ProUpgradeCard` reads `SessionProvider.isPro` via `context.watch` — badge switches FREE/PRO ✓, description and button label update live after purchase; "Upgrade to PRO" button navigates to `/paywall` which contains the plan selector
- Legal links (Privacy Policy + Terms of Use) live in `SettingsLegalLinks` widget — always visible on Settings screen below `ProUpgradeCard`, no gate; both open via `url_launcher` in `LaunchMode.externalApplication`; same links also appear in `PaywallCta` fine-print row
